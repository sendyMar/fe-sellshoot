...
export function ExtractionResultCard({ result }: { result: any }) {
  const isStock = result.screenshot.tag === 'product_stock';
  const isOrder = result.screenshot.tag === 'order_list' || result.screenshot.tag === 'order_detail';
    ...
    <div className="flex flex-col gap-4 p-4 border border-slate-200 rounded-xl bg-white shadow-sm sm:flex-row">
      ...
      <div className="flex-1 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">
              {result.screenshot.platform} • {result.screenshot.tag?.replace('_', ' ')}
            </span>
        ...
        <div className="space-y-3">
          {result.narrative && (
            <div className="p-3 bg-violet-50 border border-violet-100 rounded-lg text-sm text-violet-800">
              <span className="font-semibold block mb-1">Narasi AI:</span>
              {result.narrative}
            </div>
          )}

          {result.screenshot.tag !== 'chat' && (
            <>
              <h4 className="text-sm font-medium text-slate-900 mt-4">Items Ditemukan: {result.items.length}</h4>
                  ...
                  <table className="w-full text-sm text-left">
                    <thead className="bg-slate-50 text-slate-500 text-xs">
                      <tr>
                        ...
                        {isOrder && <th className="px-4 py-2 font-medium">Status</th>}
                        <th className="px-4 py-2 font-medium">{isStock ? 'Stok' : 'Qty'}</th>
                        {isOrder && <th className="px-4 py-2 font-medium">Catatan</th>}
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {result.items.map((item: any) => (
                        <tr key={item.id} className="bg-white hover:bg-slate-50">
                          ...
                          {isOrder && (
                            <td className="px-4 py-3 text-slate-500 max-w-[150px] truncate text-xs" title={item.notes}>
                              {item.notes || '-'}
                            </td>
                          )}
